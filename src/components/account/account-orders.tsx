...
/**
 * Full order history for this device.
 *
 * Status is shown as "Confirmation pending" on every order and nothing more —
 * there is no courier feed behind it, and inventing a "Shipped" badge nobody
 * updates would be a lie the customer acts on. When an API exists, the badge
 * reads `order.status` and the rest of this screen is unchanged.
 */
export function AccountOrders() {
  const { orders, ready } = useDeviceOrders();
  ...
  return (
    <div className="pb-4">
      ...
      <p className="mt-1.5 text-[14px] text-ink-soft">
        {orders.length === 0
          ? "You have not placed an order on this device yet."
      ...
      {orders.length === 0 ? (
        <div className="mt-5 flex flex-col items-center gap-3 rounded-[var(--radius-md)] border border-line bg-subtle px-6 py-14 text-center">
          ...
          <p className="max-w-sm text-[14px] text-ink-muted">
            Orders placed on this phone or computer show up here. Ordered somewhere
            else? Track it with your order number.
          </p>
        ...
        <>
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 border-b border-line px-4 py-3">
                  <div className="min-w-0">
                    <p className="tabular text-[14px] font-semibold">{o.id}</p>
                    <p className="mt-0.5 text-[12px] text-ink-muted">
                      {formatOrderDate(o.placedAt)} ·{" "}
                      {o.area === "inside-dhaka" ? "Inside Dhaka" : "Outside Dhaka"}
                    </p>
                  </div>
                  <span className="inline-flex items-center rounded-full bg-brand-tint px-2.5 py-1 text-[10px] font-semibold tracking-wide text-brand uppercase">
          ...
          <p className="mt-5 flex flex-wrap items-center gap-2 rounded-[var(--radius-sm)] bg-subtle px-3.5 py-3 text-[13px] text-ink-soft">
            ...
            <span>
              Ordered from another device? Call{" "}
              <a
                href={`tel:${site.phone}`}
